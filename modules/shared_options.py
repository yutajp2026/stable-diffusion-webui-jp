...
options_templates.update(options_section(('sampler-params', "サンプラー設定", "sd"), {
    ...
    'sgm_noise_multiplier': OptionInfo(False, "SGMノイズ乗数", infotext='SGM noise multiplier').link("PR", "https://github.com/AUTOMATIC1111/stable-diffusion-webui/pull/12818").info("公式 SDXL 実装に初期ノイズを合わせる - 画像再現にのみ有用"),
    'uni_pc_variant': OptionInfo("bh1", "UniPC バリアント", gr.Radio, {"choices": ["bh1", "bh2", "vary_coeff"]}, infotext='UniPC variant'),
    'uni_pc_skip_type': OptionInfo("time_uniform", "UniPC スキップタイプ", gr.Radio, {"choices": ["time_uniform", "time_quadratic", "logSNR"]}, infotext='UniPC skip type'),
    'uni_pc_order': OptionInfo(3, "UniPCオーダー", gr.Slider, {"minimum": 1, "maximum": 50, "step": 1}, infotext='UniPC order').info("サンプリングステップより小さくする必要あり"),
    'uni_pc_lower_order_final': OptionInfo(True, "UniPC 下位オーダー最終", infotext='UniPC lower order final'),
