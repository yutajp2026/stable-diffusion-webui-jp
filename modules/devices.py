...
def test_for_nans(x, where):
    ...
    if where == "unet":
        message = "UnetでNaNを含むテンソルが生成されました。"

        if not shared.cmd_opts.no_half:
            message += " これは、画像を表現するための精度が不足しているか、またはあなたのビデオカードがハーフ型をサポートしていないためのどちらかです。これを修正するには、設定 > Stable Diffusion で\"Upcast cross attention layer を float32 に設定\"オプションを有効にするか、またはコマンドライン引数に --no-half を使用してみてください。"

    elif where == "vae":
        message = "VAEでNaNを含むテンソルが生成されました。"

        if not shared.cmd_opts.no_half and not shared.cmd_opts.no_half_vae:
            message += " これは、画像を表現するための精度が不足しているか、またはあなたのビデオカードがハーフ型をサポートしていないためのどちらかです。これを修正するには、設定 > Stable Diffusion で\"Upcast cross attention layer を float32 に設定\"オプションを有効にするか、またはコマンドライン引数に --no-half-vae を使用してみてください。"
    else:
        message = "NaNを含むテンソルが生成されました。"

    message += " このチェックを無効にするには、--disable-nan-check コマンドライン引数を使用してください。"
...
def force_model_fp16():
    ...
    print("`--precision half` のため、ldm/sgm GroupNorm32 は通常の torch.nn.GroupNorm に置き換えられました。")
