    ...
    try:
        ...
        if command_path is not None and command_path != model_path:
            ...
            if os.path.exists(pretrained_path):
                print(f"パスを追加する: {pretrained_path}")
        ...
        for place in places:
            for full_path in shared.walk_files(place, allowed_extensions=ext_filter):
                if os.path.islink(full_path) and not os.path.exists(full_path):
                    print(f"壊れたシンボリックリンクをスキップしています: {full_path}")
                    continue
...
def load_spandrel_model(
    ...
    *,
    ...
    if expected_architecture and arch.name != expected_architecture:
        logger.warning(
            f"モデル {path!r} は {expected_architecture!r} モデルではありません（{arch.name!r} が得られました）",
        )
    half = False
    if prefer_half:
        if model_descriptor.supports_half:
            ...
            logger.info("モデル %s はハーフ精度をサポートしていません、--half を無視します", path)
    if dtype:
        model_descriptor.model.to(dtype=dtype)
    model_descriptor.model.eval()
    logger.debug(
        "%s を %s から読み込みました (device=%s, half=%s, dtype=%s)",
