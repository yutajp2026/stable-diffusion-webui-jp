...
if cmd_opts.ngrok is not None:
    ...
    print('ngrok認証トークンが検出されました。接続を試みています...')
    ngrok.connect(
        cmd_opts.ngrok,
...
def create_ui():
                    ...
                    if category == "prompt":
                        ...
                        with FormRow():
                            ...
                            with gr.Column(elem_id="txt2img_dimensions_row", scale=1, elem_classes="dimensions-tools"):
                                res_switch_btn = ToolButton(value=switch_values_symbol, elem_id="txt2img_res_switch_btn", tooltip="幅/高さを切り替え")
                        ...
                        with gr.Row(elem_id="txt2img_accordions", elem_classes="accordions"):
                            with InputAccordion(False, label="高解像度修正", elem_id="txt2img_hr") as enable_hr:
                                with enable_hr.extra():
                                    hr_final_resolution = FormHTML(value="", elem_id="txtimg_hr_finalres", label="アップスケールされた解像度", interactive=False, min_width=0)
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row1", variant="compact"):
                                    ...
                                    hr_second_pass_steps = gr.Slider(minimum=0, maximum=150, step=1, label='高解像度ステップ', value=0, elem_id="txt2img_hires_steps")
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row3", variant="compact", visible=opts.hires_fix_show_sampler) as hr_sampler_container:
                                    ...
                                    hr_sampler_name = gr.Dropdown(label='高解像度サンプリング方法', elem_id="hr_sampler", choices=["Use same sampler"] + sd_samplers.visible_sampler_names(), value="Use same sampler")
                                    hr_scheduler = gr.Dropdown(label='高解像度スケジュールタイプ', elem_id="hr_scheduler", choices=["Use same scheduler"] + [x.label for x in sd_schedulers.schedulers], value="Use same scheduler")
                        ...
                        if not opts.dimensions_and_batch_together:
                            with FormRow(elem_id="txt2img_column_batch"):
                                batch_count = gr.Slider(minimum=1, step=1, label='バッチ数', value=1, elem_id="txt2img_batch_count")
                                batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='バッチサイズ', value=1, elem_id="txt2img_batch_size")
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Tab("生成", id="img2img_generation") as img2img_generation_tab, ResizeHandleRow(equal_height=False):
            with ExitStack() as stack:
                    ...
                    if category == "image":
                        with gr.Tabs(elem_id="mode_img2img"):
                            ...
                            with gr.TabItem('スケッチ', id='img2img_sketch', elem_id="img2img_img2img_sketch_tab") as tab_sketch:
                                sketch = gr.Image(label="img2imgの画像", elem_id="img2img_sketch", show_label=False, source="upload", interactive=True, type="pil", tool="color-sketch", image_mode="RGB", height=opts.img2img_editor_height, brush_color=opts.img2img_sketch_default_brush_color)
                                add_copy_image_controls('sketch', sketch)

                            with gr.TabItem('インペイント', id='inpaint', elem_id="img2img_inpaint_tab") as tab_inpaint:
                                init_img_with_mask = gr.Image(label="マスクを使ったインペイント用の画像", show_label=False, elem_id="img2maskimg", source="upload", interactive=True, type="pil", tool="sketch", image_mode="RGBA", height=opts.img2img_editor_height, brush_color=opts.img2img_inpaint_mask_brush_color)
                                add_copy_image_controls('inpaint', init_img_with_mask)

                            with gr.TabItem('インペイントスケッチ', id='inpaint_sketch', elem_id="img2img_inpaint_sketch_tab") as tab_inpaint_color:
                                inpaint_color_sketch = gr.Image(label="スケッチインペインティングの色", show_label=False, elem_id="inpaint_sketch", source="upload", interactive=True, type="pil", tool="color-sketch", image_mode="RGB", height=opts.img2img_editor_height, brush_color=opts.img2img_inpaint_sketch_default_brush_color)
                            ...
                            with gr.TabItem('インペイントアップロード', id='inpaint_upload', elem_id="img2img_inpaint_upload_tab") as tab_inpaint_upload:
                                init_img_inpaint = gr.Image(label="img2img用の画像", show_label=False, source="upload", interactive=True, type="pil", elem_id="img_inpaint_base")
                            ...
                            with gr.TabItem('バッチ', id='batch', elem_id="img2img_batch_tab") as tab_batch:
                                with gr.Tabs(elem_id="img2img_batch_source"):
                                    ...
                                    with gr.TabItem('ディレクトリから', id='batch_from_dir', elem_id="img2img_batch_from_dir_tab") as tab_batch_from_dir:
                                        ...
                                        img2img_batch_inpaint_mask_dir = gr.Textbox(label="インペイントバッチマスクディレクトリ (インペイントバッチ処理にのみ必要)", **shared.hide_dirs, elem_id="img2img_batch_inpaint_mask_dir")
                        ...
                        with gr.Row():
                            ...
                            image_cfg_scale = gr.Slider(minimum=0, maximum=3.0, step=0.05, label='イメージCFGスケール', value=1.5, elem_id="img2img_image_cfg_scale", visible=False)
                        ...
                        with FormGroup(elem_id="inpaint_controls", visible=False) as inpaint_controls:
                            ...
                            with FormRow():
                                inpainting_fill = gr.Radio(label='マスクされた領域の内容', choices=['埋めて', 'オリジナル', '潜在ノイズ', '潜在的な無'], value='オリジナル', type="index", elem_id="img2img_inpainting_fill")
    ...
    with gr.Blocks(analytics_enabled=False) as pnginfo_interface:
        with ResizeHandleRow(equal_height=False):
            with gr.Column(variant='panel'):
                image = gr.Image(elem_id="pnginfo_image", label="出典", source="upload", interactive=True, type="pil")
    ...
    with gr.Blocks(analytics_enabled=False) as train_interface:
        with gr.Row(equal_height=False):
            gr.HTML(value="<p style='margin-bottom: 0.7em'>詳しい説明は<a href=\"https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/Textual-Inversion\">wikiを参照してください<b></a></b>。</p>")

        with ResizeHandleRow(variant="compact", equal_height=False):
            with gr.Tabs(elem_id="train_tabs"):

                with gr.Tab(label="埋め込みを作成する", id="create_embedding"):
                ...
                with gr.Tab(label="訓練", id="train"):
                    gr.HTML(value="<p style='margin-bottom: 0.7em'>埋め込みまたはHypernetworkを学習します。1:1の比率の画像が入ったディレクトリを指定する必要があります <a href=\"https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/Textual-Inversion\" style=\"font-weight:bold;\">[wiki]</a></p>")
                    with FormRow():
                    ...
                    with FormRow():
                        ...
                        clip_grad_value = gr.Textbox(placeholder="グラデーションクリップの値", value="0.1", show_label=False)
                    ...
                    dataset_directory = gr.Textbox(label='データセットディレクトリ', placeholder="入力画像のディレクトリへのパス", elem_id="train_dataset_directory")
                    log_directory = gr.Textbox(label='ログディレクトリ', placeholder="出力を書き込むディレクトリへのパス", value="textual_inversion", elem_id="train_log_directory")
                    ...
                    training_width = gr.Slider(minimum=64, maximum=2048, step=8, label="幅", value=512, elem_id="train_training_width")
                    training_height = gr.Slider(minimum=64, maximum=2048, step=8, label="高さ", value=512, elem_id="train_training_height")
                    varsize = gr.Checkbox(label="画像のリサイズを行わない", value=False, elem_id="train_varsize")
                    steps = gr.Number(label='最大ステップ数', value=100000, precision=0, elem_id="train_steps")

                    with FormRow():
                        create_image_every = gr.Number(label='画像をログディレクトリにNステップごとに保存する、0で無効にします', value=500, precision=0, elem_id="train_create_image_every")
                        save_embedding_every = gr.Number(label='EmbeddingのコピーをログディレクトリにNステップごとに保存する、0で無効にします', value=500, precision=0, elem_id="train_save_embedding_every")

                    use_weight = gr.Checkbox(label="減量量としてPNGアルファチャネルを使います", value=False, elem_id="use_weight")

                    save_image_with_stored_embedding = gr.Checkbox(label='PNGチャンクに埋め込みした画像を保存してください', value=True, elem_id="train_save_image_with_stored_embedding")
                    ...
                    with gr.Row():
                        ...
                        train_hypernetwork = gr.Button(value="ハイパーネットワーク訓練", variant='primary', elem_id="train_train_hypernetwork")
            ...
            with gr.Column(elem_id='ti_gallery_container'):
                ...
                gr.Gallery(label='出力', show_label=False, elem_id='ti_gallery', columns=4)
